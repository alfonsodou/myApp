 ...
 * 
 */
package org.example.client.application.animation;
 ...
 *
 */
public final class MyAnimation2 extends AnimationAdapter implements TouchDownHandler, TouchUpHandler, TouchMoveHandler {
	Scene scene;
	...
	public void onStart(RenderingContext context) {
		scene = new Scene();
		camera = new PerspectiveCamera(70, // fov
		...
		MeshBasicMaterial material2 = new MeshBasicMaterial().setColor(0x000000).setOpacity(0.25).setTransparent(true);
	...
	public void onTouchMove(int screenX, int screenY, int pointer) {
		...
		if (intersects.size() > 0) {
			if (intersected != intersects.get(0).object) {
				if (intersected != null) {
					((MeshLambertMaterial) intersected.getMaterial()).getColor()
							.setHex(currentHex.get(intersected.getId() + ""));
				}

				intersected = intersects.get(0).object;
				currentHex.put(intersected.getId() + "",
						((MeshLambertMaterial) intersected.getMaterial()).getColor().getHex());
